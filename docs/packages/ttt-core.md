...
- Two server projections, deliberately separate rather than one flag-switched query: the paginated artisan gallery (`REALM_SHARED_FILES_PAGE_LIMIT`, opaque cursor, `{ files, folders, nextCursor }`) whose file rows accept only the approved standings, and the steward/admin promotion queue (`REALM_FILE_PROMOTION_QUEUE_PAGE_LIMIT`) whose rows accept only the pending standing. Folder documents are returned through the projection owner, never opened to direct client reads.
