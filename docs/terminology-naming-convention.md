...
| Guildmate | `GuildmateUser` / `guildmateUser` / `guildmateUsers` / `guildmateUserIds` | `activeUser` / `activeUsers` / `ActiveUsersSection` / `member` / `projectMember` / `memberUid` | One person in a Guild. Used for Work-membership references everywhere the old code said "active user", "project user", or "member". The bare word `member` is NOT a kept exception — it renames to `guildmateUser` everywhere (see edge case below). |
...
### The `member` noun → `guildmateUser`

The bare word `member` (a person in a Work's guild) is NOT a kept exception — it is the old code identifier for the Guildmate concept and renames to `guildmateUser` everywhere a code identifier is involved:

- Permission action ids: `member.guildStanding.update` → `guildmateUser.guildStanding.update`; `member.tradeProfession.update` → `guildmateUser.tradeProfession.update`.
- Audit event types: `workProject.memberGuildStandingChanged` → `workProject.guildmateUserGuildStandingChanged`; `workProject.memberTradeProfessionsChanged` → `workProject.guildmateUserTradeProfessionsChanged`.
- React Query key segments/methods: `workProjectKeys.members` / segment `'members'` → `guildmateUsers`.
- Firestore rules helpers: `memberHasAnyRole` → `guildmateUserHasAnyGuildStanding`, `isProjectMember` → `isGuildmateUser`, `isActiveProjectMember` → `isActiveGuildmateUser`, `memberData` → `guildmateUserData`.
- Backend helpers/locals: `readProjectMember` → `readGuildmateUser`, `projectMembers` → `guildmateUsers`, `memberUid` → `guildmateUserUid`.

(The permission-action and audit-type strings above live in `@ttt-productions/ttt-core` and rename in the package first, then consumers adopt.) UI-facing copy still uses the short word "Guildmate" (or "member" only where it's plain English prose, never as a code identifier).
...
Note: there is no generic `UsersSection` wrapper exception — the guildmate section is `GuildmateUsersSection` (in `guildmate-users-section.tsx`) and any thin wrapper is renamed/inlined to match; do not reintroduce a generic `UsersSection` name.
