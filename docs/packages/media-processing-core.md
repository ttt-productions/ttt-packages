...
**`isObjectNotFoundError(e)`** is the one "the object does not exist" classifier, for both backends: a Cloud Storage `404` code (numeric or string) or "No such object" message, and the R2 store's `NoSuchKey` (an R2 `NoSuchBucket` is not a missing object). The Firebase store's `delete` and `readStagedUploadMetadata` use it; a consumer deciding whether a failed store read, or its own Admin-SDK Storage call, hit a missing object calls it rather than re-testing codes and messages.
