...
`MediaObjectStore` (`src/server/storage-ops.ts`) is the chokepoint for every final-media write, copy, read-back, and delete: `putFile` (content type + immutable cache-control), `copy` (server-side, never overwrites), `readToFile` (streamed to disk), and `delete` (a missing object is a no-op). Writes return object keys, never URLs; display URLs are built at render time by the consuming app.
