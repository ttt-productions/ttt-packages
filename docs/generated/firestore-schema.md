...
Source of truth: `COLLECTION_SCHEMAS` in `@ttt-productions/ttt-core/doc-schemas`. 151 collections bound. Regenerated by `scripts/generate-schema-docs.mjs`.
...
| `parentWaitStartedAt` | `custom` | yes |
| `createdAt` | `custom` |  |
...
## `privilegedReviewerCapabilityGrants/{uid}`

| Field | Type | Optional |
| --- | --- | --- |
| `schemaVersion` | `1` |  |
| `uid` | `string` |  |
| `grantedCapabilities` | `'nciiRequestReviewer' \| 'nciiEvidenceReviewer' \| 'nciiAppealReviewer' \| 'childSafetyReviewer' \| 'legalDispositionAuthority'[]` |  |
| `grantedByUid` | `string` | yes |
| `createdAt` | `number` |  |
| `updatedAt` | `number` |  |

## `privilegedReviewerPasskeyProfiles/{uid}`

| Field | Type | Optional |
| --- | --- | --- |
| `schemaVersion` | `1` |  |
| `uid` | `string` |  |
| `credentials` | `{ credentialId, publicKey, signCount, transports, aaguid, label, createdAt, lastUsedAt }[]` |  |
| `pendingChallenge` | `{ challenge, type, createdAt, expiresAt }` | yes |
| `grantExpiresAt` | `number` | yes |
| `twoStepReauthCapabilities` | `'nciiRequestReviewer' \| 'nciiEvidenceReviewer' \| 'nciiAppealReviewer' \| 'childSafetyReviewer' \| 'legalDispositionAuthority'[]` |  |
| `createdAt` | `number` |  |
| `updatedAt` | `number` |  |
